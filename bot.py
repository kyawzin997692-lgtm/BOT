...
async def handle_key(message):
    ...
    # Admin ဖြစ်ပါက Key စစ်ဆေးစရာမလိုဘဲ အလိုအလျောက်ခွင့်ပြုမည်
    if str(chat_id) == ADMIN_ID:
        approve[chat_id] = True
        user_data[chat_id] = {}
        await bot.reply_to(
            message,
            "👑 Admin ဖြင့် ဝင်ရောက်ထားပါသည်။ Key ကျော်ပြီးပါပြီ။ /input ဖြင့် Session URL ထည့်ပါ။"
        )
        return
...
async def handle_result(message):
    chat_id = message.chat.id
    if str(chat_id) == ADMIN_ID or str(chat_id) in (await get_file_content("auth_list.json"))[0]:
...
async def recheck(message):
    ...
    auth_list, _ = await get_file_content("auth_list.json")
    if str(chat_id) == ADMIN_ID or str(chat_id) in auth_list:
...
async def scan(message):
    ...
    # Admin ဖြစ်ပါက Key စစ်ဆေးမှုကို ကျော်မည်
    if str(chat_id) == ADMIN_ID:
        approve[chat_id] = True
        if chat_id not in user_data:
            user_data[chat_id] = {}
...
async def run_bruteforce(mode, chat_id, session_url, scan_id, message=None, progress_msg=None):
    ...
    try:
        while True:
            ...
            # Admin ဖြစ်ပါက သက်တမ်းစစ်ဆေးခြင်းမှ ကင်းလွတ်ခွင့်ပေးမည်
...
async def perform_check(session_url, code, chat_id, scan_id=None, recheck=False, message=None):
    ...
    post_url = base64.b64decode(
        b'aHR0cHM6Ly9wb3J0YWwtasucnVpamllbmV0d29ya3MuY29tL2FwaS9hdXRoL3ZvdWNoZXIvP2xhbmc9ZW5fVVM='
    ).decode()
