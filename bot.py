...
async def start(message):
    ...
    # Admin ဖြစ်ပါက Key မလိုဘဲ အလိုအလျောက် သုံးခွင့်ပေးမည်
...
🎉 မင်္ဂလာပါခင်ဗျာ! (ADMIN / PAID USER)
...
async def callback_handler(call):
    ...
    # Admin ဖြစ်ပါက အမြဲတမ်းခွင့်ပြုရန် သတ်မှတ်ချက်
    is_user_allowed = is_admin(user_id) or user_id in paid_users or user_id in approve
...
✅ ACCESS GRANTED - Unlimited Access"""
        else:
            text = f"""✨ STAR LINK CODE HACK ✨
    ...
    if call.data == "menu_enter_userid":
        if is_admin(user_id):
            ...
            await bot.edit_message_text(
                chat_id=chat_id,
                message_id=call.message.message_id,
                text=f"✅ Admin အဖြစ် အလိုအလျောက် အတည်ပြုပြီးပါပြီ။\n\nUSER ID: {user_id}",
                reply_markup=get_main_keyboard()
            )
            await bot.answer_callback_query(call.id)
            return
...
async def handle_key(message):
    ...
    if is_admin(user_id):
        approve[message.chat.id] = True
        paid_users[user_id] = True
        await bot.reply_to(message, "✅ Admin ဖြစ်၍ Key ထည့်စရာမလိုဘဲ အပြည့်အဝ အသုံးပြုနိုင်ပါသည်။")
        return
...
async def get_session_id(session, session_url, previous_session_id=None):
    ...
    headers = {
        ...
        'cookie': 'sensorsdata2015jssdkcross=%7B%22distinct_id%22%3A%2219e0ddbd9f2152-0df941f2efc6b08-4c657b58-1327104-19e0ddbd9f3a60%22%2C%22first_id%22%3A%22%22%2C%22props%22%3A%7B%22%24latest_traffic_source_type%22%3A%22%E8%87%AA%E7%84%B6%E6%90%9C%E7%B4%A2%E6%B5%81%E9%87%8F%22%2C%22%24latest_search_keyword%22%3A%22%E6%9C%AA%E5%8F%96%E5%8%B0%E5%80%BC%22%2C%22%24latest_referrer%22%3A%22https%3A%2F%2Fgemini.google.com%2F%22%7D%2C%22identities%22%3A%22eyIkaWRlbnRpdHlfY29va2llX2lkIjoiMTllMGRkYmQ5ZjIxNTItMGRmOTQxZjJlZmM2YjA4LTRjNjU3YjU4LTEzMjcxMDQtMTllMGRkYmQ5ZjNhNjAifQ%3D%3D%22%2C%22history_login_id%22%3A%7B%22name%22%3A%22%22%2C%22value%22%3A%22%22%7D%2C%22%24device_id%22%3A%2219e0ddbd9f2152-0df941f2efc6b08-4c657b58-1327104-19e0ddbd9f3a60%22%7D'
    }
...
async def start_polling():
    ...
    while True:
        try:
            # Webhook conflict error ကို ဖြေရှင်းရန် အလိုအလျောက် ဖြုတ်ပေးခြင်း
